...
require get_template_directory() . '/framework/classes/lib/Less/Cache.php';
...
class Less_Parser{
	...
	private function GetRules( $file_path ){
		...
		if( $cache_file && file_exists($cache_file) ){
			switch(Less_Parser::$options['cache_method']){
				...
				case 'serialize':
					$cache = unserialize(lessc_helper::fs_get_contents($cache_file));
					if( $cache ){
						touch($cache_file);
						$this->UnsetInput();
						return $cache;
	...
	public function SetInput( $file_path ){
		...
		if( $file_path ){
			$this->input = lessc_helper::fs_get_contents( $file_path );
		}
	...
	private function parseImport(){
		...
		$dir = $this->MatchReg('/\\G@(i|I)mport?\s+/');
